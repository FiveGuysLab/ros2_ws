...
LOG_DIR="/home/guy/test_logs/busywait_pubsub_node_wo_timer"
...
for iteration in {1..3}; do
  ...
  timeout 80s ros2 run busywait_pubsub_node_wo_timer busywait_pubsub_node_wo_timer &
  NODE_PID=$!
  ...
  cp "$RESULT_FILE" "$LOG_DIR_WITH_TIME/bw_wo_timer_${iteration}.txt"
