...
LOG_DIR="/home/guy/test_logs/busywait_pubsub_node"
...
for iteration in {1..3}; do
  ...
  timeout 80s ros2 run busywait_pubsub_node busywait_pubsub_node &
  NODE_PID=$!
  ...
  cp "$RESULT_FILE" "$LOG_DIR_WITH_TIME/bw_${iteration}.txt"
