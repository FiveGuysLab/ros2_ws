#include "multithread_priority_test_node/scalable_subscriber.hpp"
#include "multithread_priority_test_node/scalable_publisher.hpp"
...
void print_usage(const char* program_name)
{
  std::cout << "Usage: " << program_name << " [options]\n"
            ...
            << "  --threads <num>        Number of executor threads (default: 4)\n"
...
int main(int argc, char * argv[])
{
  ...
  // Set up the multithread priority executor with priority memory strategy
  rclcpp::ExecutorOptions options;
  auto strategy = std::make_shared<PriorityMemoryStrategy<>>();
  options.memory_strategy = strategy;
  auto next_exec_timeout = std::chrono::nanoseconds(-1); // Default timeout
  auto executor = std::make_unique<timed_executor::MultithreadTimedExecutor>(
    options, 
    "multithread_scalability_priority_executor", 
    num_threads,
    next_exec_timeout
  );
  ...
  std::cout << "\n=== Multithread Scalability Priority Test Configuration ===" << std::endl;
  ...
  RCLCPP_INFO(rclcpp::get_logger("multithread_priority_test"), "Starting multithread scalability priority test...");
